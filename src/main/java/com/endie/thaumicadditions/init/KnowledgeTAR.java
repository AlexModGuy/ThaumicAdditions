...
public class KnowledgeTAR
{
	...
	private static void $init()
	{
		...
		new REB().setBaseInfo("TAR_AURA_DISPERSER", "aura_disperser", 0, -2, new ItemStack(BlocksTAR.AURA_DISPERSER)).setStages(new RSB().setText("research_stage.aura_disperser.1").setKnow(new Knowledge(EnumKnowledgeType.OBSERVATION, TAReconstructed.RES_CAT, 1), new Knowledge(EnumKnowledgeType.THEORY, ResearchCategories.getResearchCategory("ALCHEMY"), 1)).setConsumedItems(AspectUtil.salt((Aspect) null)).build(), new RSB().setText("research_stage.aura_disperser.2").setRecipes(InfoTAR.MOD_ID + ":aura_disperser").build()).setParents("BASEARTIFICE", "METALLURGY@2", "TAR_ESSENCE_SALT").buildAndRegister();
		
		new REB().setBaseInfo("TAR_CRYSTAL_BORE", "crystal_bore", 3, 0, new ItemStack(BlocksTAR.CRYSTAL_BORE)).setMeta(EnumResearchMeta.SPIKY).setStages(new RSB().setText("research_stage.crystal_bore.1").setConsumedItems(new ItemStack(ItemsTC.crystalEssence)).setKnow(new Knowledge(EnumKnowledgeType.OBSERVATION, ResearchCategories.getResearchCategory("ALCHEMY"), 1), new Knowledge(EnumKnowledgeType.OBSERVATION, ResearchCategories.getResearchCategory("GOLEMANCY"), 1)).build(), new RSB().setText("research_stage.crystal_bore.2").setRecipes(InfoTAR.MOD_ID + ":crystal_bore").build()).setParents("TAR_THAUMADDS").buildAndRegister();
		new REB().setBaseInfo("TAR_CRYSTAL_WATER", "crystal_water", -1, 1, FluidUtil.getFilledBucket(new FluidStack(FluidsTAR.CRYSTAL_WATER, Fluid.BUCKET_VOLUME))).setStages(new RSB().setText("research_stage.crystal_water.1").setConsumedItems(new ItemStack(ItemsTC.crystalEssence), new ItemStack(Items.WATER_BUCKET)).setKnow(new Knowledge(EnumKnowledgeType.OBSERVATION, ResearchCategories.getResearchCategory("ALCHEMY"), 1)).build(), new RSB().setText("research_stage.crystal_water.2").setRecipes(InfoTAR.MOD_ID + ":crystal_water", InfoTAR.MOD_ID + ":mb.crystal_acceleration").build()).setParents("TAR_THAUMADDS").buildAndRegister();
